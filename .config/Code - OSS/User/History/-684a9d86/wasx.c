...
commitnotify(struct wl_listener *listener, void *data)
{
	...
	if (client_surface(c)->mapped && c->mon) {
		if (c->isfloating || !c->mon->lt[c->mon->sellt]->arrange) {
			...
			c->geom.width = geom.width + 2 * c->bw;
			c->geom.height = geom.height + 2 * c->bw;
		}
...
#endif
	/* Use the raw keycode to store state and translate to xkbcommon later*/
	/* Get a list of keysyms based on the keymap for this keyboard */
	const xkb_keysym_t *syms;
	/* Translate libinput keycode -> xkbcommon */
	int nsyms = xkb_state_key_get_syms(
			group->wlr_group->keyboard.xkb_state, event->keycode + 8, &syms);
...
void
keypressglobal(struct wlr_surface *last_surface,
			   struct wlr_keyboard *keyboard,
			   ...
			   uint32_t mods, xkb_keysym_t keysym)
{
	...
	for (size_t r = 0; r < LENGTH(pass_rules); r++) {
		if (xkb_keysym_to_upper(pass_rules[r].key) != xkb_keysym_to_upper(keysym))
			continue;
		...
		if (keysym == XKB_KEY_XF86Forward && (strncmp(appid, "teamspeak", 9) == 0) || strncmp(appid, "discord", 7) == 0) {
		...
		if (target != lastc) {
			wlr_seat_keyboard_notify_enter(seat, client_surface(target),
								  NULL, 0, &keyboard->modifiers);

			wlr_seat_keyboard_notify_key(seat, event->time_msec,
								event->keycode, event->state);

			if (last_surface) {
				const uint32_t *pressed = keyboard->keycodes;
				size_t pressed_count = keyboard->num_keycodes;
				if (pressed_count > WLR_KEYBOARD_KEYS_CAP)
					pressed_count = WLR_KEYBOARD_KEYS_CAP;

				wlr_seat_keyboard_notify_enter(seat, last_surface,
